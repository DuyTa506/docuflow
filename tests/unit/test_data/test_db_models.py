...
# Document.id has no server-side default — IdGenerator runs only in service layer,
# so tests must supply an explicit id.
def _doc(**kwargs):
    defaults = dict(
        id=str(uuid.uuid4()),
        title="test.pdf",
...
class TestTreeIndex:
    ...
    def test_tree_index_timestamp(self, test_db_session):
        ...
        tree_index = TreeIndex(document_id=doc.id, tree_data={}, config={})
