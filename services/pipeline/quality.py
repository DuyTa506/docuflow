...
def build_quality_report(document_id: str) -> dict[str, Any]:
    ...
    with db_manager.session() as db:
        ...
        if summary and summary.content:
            abstract_sentences = len(
                [s for s in summary.content.replace("!", ".").replace("?", ".").split(".") if s.strip()]
            )

        warnings: list[str] = []
        if not has_tree:
            warnings.append("Không có TreeIndex — tóm tắt/nội dung có thể dùng fallback chunk/heading")
        elif not tree_used:
