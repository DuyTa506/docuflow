...
try:
    ...
    get_doclayout_onnx_model_path = None  # type: ignore
...
class OnnxModel(DocLayoutModel):
    ...
    @staticmethod
    def from_pretrained(model_path: str | None = None):
        if model_path:
            return OnnxModel(model_path)
        try:
            if get_doclayout_onnx_model_path is not None:
                pth = get_doclayout_onnx_model_path()
            else:
                raise ImportError("babeldoc not installed")
        except Exception:
            from config.settings import settings

            if settings.doclayout_model_path:
                pth = settings.doclayout_model_path
            else:
                raise RuntimeError(
                    "DocLayout ONNX model not found. Install babeldoc or set DOCLAYOUT_MODEL_PATH."
                )
