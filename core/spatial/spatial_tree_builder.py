...
def predict_hierarchy_spatial(
    elements: List[Dict], page_dims: Dict[str, int], spatial_weights: Optional[Dict] = None
) -> List[Dict]:
    """
...
def _is_docx_elements(elements: List[Dict]) -> bool:
    ...
    return not any(
        elem.get("bbox_x1", 0)
        or elem.get("bbox_y1", 0)
        or elem.get("bbox_x2", 0)
        or elem.get("bbox_y2", 0)
        for elem in elements
    )
...
def build_spatial_tree(
    ...
    if use_filters:
        try:
            ...
            current_elements, removed = apply_all_filters(
                current_elements, filter_repeated=True, filter_noise=True, filter_margins=False
            )
    ...
    if use_zone_classification:
        try:
            ...
            current_elements = classify_zones_batch(current_elements, page_dims, cross_page_stats)
        except ImportError:
            pass
    ...
    if use_reading_order:
        try:
            ...
            current_elements = sorted(
                current_elements,
                key=lambda e: (e.get("page_number", 1), e.get("bbox_y1", e.get("y1", 0))),
            )

    # Phase 4: Hierarchy prediction (spatial-based)
    current_elements = predict_hierarchy_spatial(current_elements, page_dims, spatial_weights)
    ...
    if use_thinning:
        ...
        thinning_stats = {
            ...
            "reduction_percent": (
                round((nodes_before - nodes_after) / nodes_before * 100, 1)
                if nodes_before > 0
                else 0
            ),
        }
