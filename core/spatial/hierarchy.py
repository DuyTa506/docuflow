...
def calculate_adaptive_thresholds(
    elements: List[Dict], weights: Optional[Dict[str, float]] = None
) -> Dict[int, float]:
    """
...
def classify_elements_with_metadata(
    ...
    for elem in layout_elements:
        hierarchy_level = predict_hierarchy_level(elem, page_width, page_height, weights)
...
def get_page_dimensions_from_elements(elements: List[Dict]) -> Dict[str, int]:
    ...
    # Add some padding
    return {"width": int(max_x * 1.1), "height": int(max_y * 1.1)}
