...
def analyze_cross_page_repetitions(
    elements: List[Dict], min_pages: int = 3, text_min_length: int = 3
...
def filter_margin_elements(
    elements: List[Dict], margin_ratio: float = 0.05, page_dims: Optional[Dict[str, int]] = None
) -> Tuple[List[Dict], List[Dict]]:
    """
...
def apply_all_filters(
    ...
    if filter_repeated:
        current, removed = filter_repeated_elements(current, min_pages=min_repeat_pages)
        removed_by_filter["repeated"] = removed

    # Filter 2: Noise elements
    if filter_noise:
        current, removed = filter_noise_elements(
            current, min_area_ratio=min_area_ratio, max_area_ratio=max_area_ratio
        )
