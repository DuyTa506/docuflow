...
def detect_columns_projection(
    ...
    for i, valley_x in enumerate(valleys):
        columns.append(Column(x1=prev_x, x2=valley_x, width=valley_x - prev_x, index=len(columns)))
        prev_x = valley_x

    # Add final column
    columns.append(Column(x1=prev_x, x2=page_width, width=page_width - prev_x, index=len(columns)))
...
def group_into_lines(
    elements: List[Dict], vertical_tolerance: Optional[float] = None
) -> List[List[Dict]]:
    """
...
def group_lines_to_blocks(
    ...
    for i in range(1, len(lines)):
        ...
        prev_bottom = max(elem.get("bbox_y2", elem.get("y2", 0)) for elem in prev_line)

        # Get top of current line
        curr_top = min(elem.get("bbox_y1", elem.get("y1", 0)) for elem in curr_line)
...
def create_block_from_lines(lines: List[List[Dict]], block_index: int) -> Block:
...
def link_captions_to_figures(elements: List[Dict], max_distance_ratio: float = 0.15) -> List[Dict]:
    """
...
def process_page_layout(
    ...
    """
    result = {"columns": [], "blocks": [], "elements": elements}
    ...
    columns = [
        Column(x1=0, x2=page_dims.get("width", 800), width=page_dims.get("width", 800), index=0)
    ]

    if detect_multi_column:
        columns = detect_columns_projection(elements, page_dims.get("width", 800))
    ...
    if group_blocks:
        ...
        for col in columns:
            col_elements = [e for e in elements if e.get("column_index", 0) == col.index]
