...
def should_read_before(
    elem_a: Dict, elem_b: Dict, same_column_threshold: float = 0.3, same_row_threshold: float = 0.3
) -> Optional[bool]:
    """
...
def build_reading_order_graph(
    elements: List[Dict], include_zone_priority: bool = True
) -> Dict[str, List[Edge]]:
    """
...
def break_cycles(graph: Dict[str, List[Edge]], elements: List[Dict]) -> Dict[str, List[Edge]]:
    """
    ...
    while cycles:
        ...
        if worst_edge:
            ...
            graph[source_id] = [e for e in graph[source_id] if e.target_id != target_id]
...
def topological_sort(graph: Dict[str, List[Edge]], elements: List[Dict]) -> List[str]:
    """
    ...
    queue = sorted([node for node in in_degree if in_degree[node] == 0], key=get_sort_key)
...
def get_reading_order(
    elements: List[Dict], include_zone_priority: bool = True, break_cycles_enabled: bool = True
) -> List[Dict]:
    """
...
def get_reading_order_by_page(
    ...
    for page_num in sorted(pages.keys()):
        result[page_num] = get_reading_order(pages[page_num], include_zone_priority)

    return result


def flatten_reading_order(pages_order: Dict[int, List[Dict]]) -> List[Dict]:
    """
